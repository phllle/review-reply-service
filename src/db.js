...
/** Atomically claim a due reply before posting it. Returns null if cancel/send won first. */
export async function claimPendingReplyDueToSend(id, now = new Date()) {
  const res = await getPool().query(
...
export async function markPendingReplySent(id) {
  await getPool().query(
    "UPDATE pending_replies SET sent_at = NOW(), send_error = NULL WHERE id = $1 AND cancelled_at IS NULL",
    [id]
  );
}
