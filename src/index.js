...
import { renderProPage, proScript } from "./views/proPage.js";
import { escapeHtml, darkShellHtml } from "./views/shell.js";
import {
...
} from "./views/adminPage.js";
import {
  renderContactPage,
  renderCompliancePage,
  renderPrivacyPage,
  renderTermsPage
} from "./views/publicPages.js";
import {
  getPlanAmountsCents,
  computeMrr,
  computeFunnel,
...
app.patch("/businesses/:accountId", async (req, res, next) => {
  try {
    ...
    // Multi-location: target a specific location when provided (admin edits one
    // row per location); otherwise default to the account's primary location.
    const locationId = req.body?.locationId != null && String(req.body.locationId).trim()
      ? String(req.body.locationId).trim()
      : null;
    const existing = await getBusiness(accountId, locationId);
    if (!existing) {
      return res.status(404).json({ error: "Business not found. Connect via /auth/google first." });
    }
    ...
    const updated = await getBusiness(accountId, existing.locationId ?? locationId);
    res.json(updated);
  } catch (err) {
    next(err);
  }
...
app.get("/pro", async (req, res, next) => {
  try {
    ...
    res.set("Content-Type", "text/html; charset=utf-8");
    res.send(renderProPage({ accountId, birthday, escapeHtml }));
  } catch (err) {
    next(err);
  }
});
...
app.get("/admin/metrics", async (req, res, next) => {
  try {
    if (!(process.env.ADMIN_SECRET || "").trim()) {
      res.status(503).set("Content-Type", "text/html; charset=utf-8");
      return res.send(renderAdminDisabledHtml());
    }
    if (!isValidAdminRequest(req)) {
      res.status(401).set("Content-Type", "text/html; charset=utf-8");
...
app.get("/admin.js", (req, res) => {
  ...
  res.set("Content-Type", "application/javascript; charset=utf-8");
  res.send(adminScript(adminShowProScaleTier));
});
